...
class ContentImporter implements ContentImporterInterface {
   ...
   *
   * @var \Drupal\schwab_content_sync\SchwabContentSyncFieldProcessorPluginManagerInterface
   */
  protected SchwabContentSyncFieldProcessorPluginManagerInterface $fieldProcessorPluginManager;

  /**
   * The entity base fields processor plugin manager.
   *
   * @var \Drupal\schwab_content_sync\SchwabContentSyncBaseFieldsProcessorPluginManagerInterface
   */
  protected SchwabContentSyncBaseFieldsProcessorPluginManagerInterface $entityBaseFieldsProcessorPluginManager;
   ...
   *   The time service.
   * @param \Drupal\schwab_content_sync\SchwabContentSyncFieldProcessorPluginManagerInterface $field_processor_plugin_manager
   *   The field processor plugin manager.
   * @param \Drupal\schwab_content_sync\SchwabContentSyncBaseFieldsProcessorPluginManagerInterface $entity_base_fields_processor_plugin_manager
  ...
  public function doImport(array $content): EntityInterface {
    ...
    $this->moduleHandler->alterDeprecated(
      'Deprecated as of schwab_content_sync 1.4.0; subscribe to \Drupal\schwab_content_sync\Event\ImportEvent instead. For implementing support of new entity types, implement SchwabContentSyncBaseFieldsProcessor plugin.',
  ...
  public function setFieldValue(FieldableEntityInterface $entity, string $field_name, $field_value): void {
    ...
    $this->moduleHandler->alterDeprecated('Deprecated as of schwab_content_sync 1.4.0; use ImportFieldEvent to update fields value before setting into the entity, implement SchwabContentSyncFieldProcessor plugin instead to provide support for new field types.', 'content_import_field_value', $entity, $field_name, $field_value);
  }
  ...
  public function importFromZip(string $file_real_path): void {
    ...
    $batch = [
      ...
      'operations' => [],
      'file' => '\Drupal\schwab_content_sync\ContentBatchImporter',
      'finished' => '\Drupal\schwab_content_sync\ContentBatchImporter::batchImportFinishedCallback',
    ];
    ...
    foreach ($zip->listContents() as $zip_file) {
      ...
      if (strpos($zip_file, 'assets') === 0 && is_file($original_file_path)) {
        $batch['operations'][] = [
          '\Drupal\schwab_content_sync\ContentBatchImporter::batchImportAssets',
    ...
    foreach ($zip->listContents() as $zip_file) {
      ...
      if ($this->isZipFileValid($original_file_path)) {
        ...
        $batch['operations'][] = [
          '\Drupal\schwab_content_sync\ContentBatchImporter::batchImportFile',
          [$original_file_path],
        ];
      }
    }
    ...
    $batch['operations'][] = [
      '\Drupal\schwab_content_sync\ContentBatchImporter::cleanImportDirectory',
      [$import_directory],
    ];
